...
function spawnGraphifyBuild(cwd, graphifyOutDir) {
  ...
  try {
    ...
    // Release the lock dir if the spawn never started (graphify not on PATH),
    // otherwise the empty graphify-out/ suppresses every future build attempt.
    child.on("error", () => {
      try {
        fs.rmdirSync(graphifyOutDir);
      } catch {
        /* non-empty or already gone: a real build owns it */
      }
    });
    child.unref();
  } catch {
