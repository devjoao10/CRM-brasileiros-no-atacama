...
// Anchored on the refspec: a bare /\b(main|master)\b/ also matches `feat/main-nav`.
const PUSH_TO_MAIN = /\bgit\s+push\b(?:\s+\S+)*?\s(?:HEAD:)?(?:refs\/heads\/)?(main|master)\s*$/;
const pushRef = isPush ? (command.match(PUSH_TO_MAIN)?.[1] ?? null) : null;
const pushTargetsMain = Boolean(pushRef);

if (!onMain && !pushTargetsMain) process.exit(0);

const verb = isCommit ? "commit" : "push";
const target = onMain ? branch : (pushRef ?? "main");
